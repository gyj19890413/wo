
(function (name, definition) {
    if (typeof define === 'function') {
        define(definition);
    } else {
        this[name] = definition();
    }
})('Swiper', function () {

    /**
     *
     * @param options
     * @constructor
     */
    function Swiper(options) {
        this.version = '${version}';
        this._default = {container: '.swiper', item: '.item', direction: 'vertical', activeClass: 'active', threshold: 50, duration: 300,autoplay:3000,loop:true,timer:''};
        this._options = extend(this._default, options);
        this._start = {};
        this._move = {};
        this._end = {};
        this._prev = 0;
        this._current = 1;
        this._offset = 0;
        this._goto = -1;
        this._eventHandlers = {};

        this.$container = document.querySelector(this._options.container);
        this.$items = this.$container.querySelectorAll(this._options.item);
        this.count = this.$items.length;
        
		this.clone_node();
		
		
        this._width = this.$container.offsetWidth;
        this._height = this.$container.offsetHeight;

        this._init();
        this._bind();
        this._show('1','1');
        this.auto_Play();
    }

    /**
     * initial
     * @private
     */
    Swiper.prototype._init = function () {
        var me = this;
        var width = me._width;
        var height = me._height;


        var w = width;
        var h = height * me.count;

        if (me._options.direction === 'horizontal') {
            w = width * me.count;
            h = height;
        }

        me.$container.style.width = w + 'px';
        me.$container.style.height = h + 'px';

        Array.prototype.forEach.call(me.$items, function ($item, key) {
            $item.style.width = width + 'px';
            $item.style.height = height + 'px';
        });

        me._activate(1);
    };


	/**
     * 复制节点
     */
    Swiper.prototype.clone_node = function () {    
    	if(!this._options.loop){
    		return false;
    	}
    	var arr=[]; 
    	for(var i=0;i<this.count;i++){
    		arr.push(this.$items[i]);
    	};
    	
    	var cloneNode= arr[0].cloneNode(true);
    	
    	var cloneNodeLast = arr[arr.length-1].cloneNode(true);
    	
    	arr[this.count]=cloneNode;	
    	arr.unshift(cloneNodeLast);
    	this.$container.insertBefore(cloneNodeLast,this.$items[0]);
		this.$container.appendChild(cloneNode);
		
		this.$items=arr;
		this.count+=2;
    };
    /**
     * bind event listener
     * @private
     */
    Swiper.prototype._bind = function () {
        var me = this;

        this.$container.addEventListener('touchstart', function (e) {
        	
        	me.finish_interval();
            me._start.x = e.changedTouches[0].pageX;
            me._start.y = e.changedTouches[0].pageY;

            me.$container.style['-webkit-transition'] = 'none';
            me.$container.style.transition = 'none';

        }, false);

        this.$container.addEventListener('touchmove', function (e) {
            me._move.x = e.changedTouches[0].pageX;
            me._move.y = e.changedTouches[0].pageY;

            var distance = me._move.y - me._start.y;
            var transform = 'translate3d(0, ' + (distance - me._offset) + 'px, 0)';

            if (me._options.direction === 'horizontal') {
                distance = me._move.x - me._start.x;
                transform = 'translate3d(' + (distance - me._offset) + 'px, 0, 0)';
            }

            me.$container.style['-webkit-transform'] = transform;
            me.$container.style.transform = transform;

            e.preventDefault();
        }, false);

        this.$container.addEventListener('touchend', function (e) {
            me._end.x = e.changedTouches[0].pageX;
            me._end.y = e.changedTouches[0].pageY;


            var distance = me._end.y - me._start.y;
            if (me._options.direction === 'horizontal') {
                distance = me._end.x - me._start.x;
            }

            me._prev = me._current;
            if (distance > me._options.threshold) {
                me._current = me._current === 0 ? 0 : --me._current;
            } else if (distance < -me._options.threshold) {
                me._current = me._current < (me.count - 1) ? ++me._current : me._current;
            }

            me._show(me._current);

        }, false);

        this.$container.addEventListener('transitionEnd', function (e) {
        }, false);

        this.$container.addEventListener('webkitTransitionEnd', function (e) {
            if (e.target !== me.$container) {
                return false;
            }
			me._activate(me._current);
			
            if (me._current != me._prev || me._goto > -1) {                
                var cb = me._eventHandlers.swiped || noop;
                cb.apply(me, [me._prev, me._current]);
                me._goto = -1;
            }
            e.preventDefault();
        }, false);
    };

    /**
     * show
     * @param index
     * @private
     * trans   是否动画过渡   1不用动画过渡
     */
    Swiper.prototype._show = function (index,trans) {
        this._offset = index * this._height;
        var transform = 'translate3d(0, -' + this._offset + 'px, 0)';

        if (this._options.direction === 'horizontal') {
            this._offset = index * this._width;
            transform = 'translate3d(-' + this._offset + 'px, 0, 0)';
        }

        var duration = this._options.duration + 'ms';
		if(trans=='1'){
			duration = 'none';
		}
        this.$container.style['-webkit-transition'] = duration;
        this.$container.style.transition = duration;
        this.$container.style['-webkit-transform'] = transform;
        this.$container.style.transform = transform;
        
        if(trans!='1'){
	        this.auto_Play();
        }
    };

    /**
     * activate
     * @param index
     * @private
     */
    Swiper.prototype._activate = function (index){
        var clazz = this._options.activeClass;
//      var dots = document.getElementsByClassName('dot')
        Array.prototype.forEach.call(this.$items, function ($item, key){
            $item.classList.remove(clazz);
            if (index === key) {
                $item.classList.add(clazz);
//              dots[key].style.background = '#04be02'
            } else {
//              dots[key].style.background = '#fff'
            }
        });
    };

    /**
     * goto x page
     */
    Swiper.prototype.go = function (index) {
        if(index < 0 || index > this.count - 1 || index === this._current){
            return;
        }

        if (index === 0) {
            this._current = 0;
            this._prev = 0;
        }else{
            this._current = index;
            this._prev = index - 1;
        }

        this._goto = index;
        this._show(this._current);

        return this;
    };

    /**
     * show next page
     */
    Swiper.prototype.next = function () {
    	var me =this; 
        if (this._current >this.count - 1) {
            return;
        }
                      
        this._prev = this._current;
                             
       	me._show(++me._current);
       
        return this;
    };

    /**
     *
     * @param {String} event
     * @param {Function} callback
     */
    Swiper.prototype.on = function (event, callback) {
        if (this._eventHandlers[event]) {
            throw new Error('event ' + event + ' is already register');
        }
        if (typeof callback !== 'function') {
            throw new Error('parameter callback must be a function');
        }

        this._eventHandlers[event] = callback;

        return this;
    };



	/**
     * 结束循环播放
     * 
     */
	Swiper.prototype.finish_interval= function () {
    	var me= this;
    	clearTimeout(me._options.timer);    	
    	
    };


	/**
     * 是否循环播放
     * 循环结束回到首页
     */
    Swiper.prototype.go_first= function () {
    	var me =this;
    	if(this._current!=this.count-1){
    		return false;
    	}
    	this._prev = this._current;
    	this._current = 1;
    	
    	this._activate(this._current);
    	setTimeout(function(){
	    	me._show('1','1');
    	},me._options.duration);
    	
    };

	/**
     * 上一页回到最后一页
     */
    Swiper.prototype.go_last= function () {
    	var me =this;
    	if(this._current!=0){
    		return false;
    	};
    	
    	this._prev = this._current;
    	this._current = this.count-2;
    	
    	this._activate(this._current);
    	
    	setTimeout(function(){
	    	me._show(me._current,'1');
    	},me._options.duration);
    	
    };
    
 

 	/**
     * 自动轮播
     */
    Swiper.prototype.auto_Play = function () {
    	var me= this;
    	this.go_first();
    	
    	this.go_last();
    	var timer = me._options.autoplay;
    	
    	if(timer==0||!me._options.loop){
    		return false;
    	};
    	
		me._options.timer = setTimeout(function(){
   			me.next();
		},timer);
    };
   
    /**
     * simple `extend` method
     * @param target
     * @param source
     * @returns {*}
     */
    function extend(target, source) {
        for (var key in source) {
            target[key] = source[key];
        }

        return target;
    }

    /**
     * noop
     */
    function noop() {

    }

    /**
     * export
     */
    return Swiper;
});